...
`esy-cargo` is sample package, [`hello`](https://github.com/esy/test-hello-c), packaged for [`esy`](https://esy.sh/).
...
## How to use `esy-cargo`?

`esy-cargo` can be used from both NPM and directly from Github.

### From NPM 

`esy-cargo` is deployed on NPM can be found
[here](https://www.npmjs.com/package/TODO).

You can simply run `esy add esy-cargo` to install it, or specify it in
...
{
  "dependencies": {
    "esy-cargo": "esy-packages/esy-cargo"
  }
}
```
...
```diff
  "dependencies": {
+   "esy-cargo": "esy-packages/esy-cargo#<commit hash>"
  }
```
